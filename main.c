...
int card_uart_fd;
// 刷卡机读取前需要发送的请求数据
const char card_write_data[7] = {0x03, 0xff, 0x07, 0x04, 0x01, 0x00, 0xfe};
// 刷卡机串口数据缓冲区
...
// 请求打印机时间的 url
const char *printer_time_request = "http://47.98.115.156:1010/Prog/File/GetPrintTime";
...
void card_read_thread(void)
{
    ...
    while (1)
    {
        ...
        // http_buffer = http_get(printer_time_request);
        // if (http_buffer)
        //     printf(http_buffer);
        leds_data[0] = 1;
        leds_data[1] = 0;
