...
int main(void)
{
    ...
    if(fd < 0) 
    {
        setFileLog(LOG_FILE_PATH, "[Tv poweron] error: can't open /dev/tv_uart\n");
        return -1;
    }
    ...
    if(getFileFlag(FLAG_FILE_PATH) != 'O')
    {
        ...
        {
            setFileLog(LOG_FILE_PATH, "[Tv poweron] power on finished!\n");
            //设置文件标志位为'O'
            setFileFlag(FLAG_FILE_PATH, 'O');    
        }
        else
        {
            setFileLog(LOG_FILE_PATH, "[Tv poweron] error: disconnect with device\n");
        }
...
static char sendPowerONCommand(void)
{
    ...
    {
        setFileLog(LOG_FILE_PATH, "[Tv poweron] warning: repeat send...\n");
        write(fd, send_buff[0], SEND_BUFF_SIZE);
        repeat_count++;
    }
    ...
    {
        write(fd, send_buff[order_data[i]-'0'], SEND_BUFF_SIZE);
        while (poll(&fds, 1, REC_DELAY) == 0 && repeat_count < SEND_REPEAT)
        {
            setFileLog(LOG_FILE_PATH, "[Tv poweron] warning: repeat send...\n");
            write(fd, send_buff[order_data[i]-'0'], SEND_BUFF_SIZE);
        ...
        repeat_count = 0;
        sleep(3);
        setFileLog(LOG_FILE_PATH, "[Tv poweron] command send finished!\n");
        i++;
    }
...
char getFileFlag(const char *file_path)
{
    ...
    if (pFile == NULL)
    {
        printf("[Tv poweron] error: can't open %s\n", file_path);
        return 0;
    }
    ch = fgetc(pFile);
    fclose(pFile);
